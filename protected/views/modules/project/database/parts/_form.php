    ...
    <?php foreach ($projectDatabase->getCurrentColumns() as $column):?>
        ...
        <tr>
            ...
            <?php elseif (in_array($column['type'],array('enum'))): ?>
                <td><?php echo CHtml::dropDownList('values['.$column['name'].']',$value,$column['size'],array('class'=>'form-control'));?></td>
            <?php elseif (in_array($column['type'],array('set'))): ?>
                <td><?php echo CHtml::checkBoxList('values['.$column['name'].']',$value,$column['size'],array('class'=>'form-control'));?></td>
            <?php else:?>
